...
$stmt = $conn->prepare("INSERT INTO rides (pickup_address, dropoff_address, pickup_lat, pickup_lng, dropoff_lat, dropoff_lng, total_price, customer_phone, status, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)");
$stmt->bind_param("ssdddddss", $pickup, $dropoff, $p_lat, $p_lng, $d_lat, $d_lng, $price, $customer_phone, $type);
...
if ($stmt->execute()) {
    ...
    echo json_encode(["success" => false, "error" => "Failed: " . $conn->error]);
}
?>
